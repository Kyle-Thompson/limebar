...
BarWindow::BarWindow(size_t x, size_t y, size_t w, size_t h)
    : _x(X::Instance())
    , _window(_x.generate_id())
    , _pixmap(_x.generate_id())
    , _origin_x(x)
    , _origin_y(y)
    , _width(w)
    , _height(h) {
  ...
  _x.change_property(XCB_PROP_MODE_APPEND, _window, atom_list[NET_WM_STATE],
                     XCB_ATOM_ATOM, 32, 2, &atom_list[NET_WM_STATE_STICKY]);
  _x.change_property(XCB_PROP_MODE_REPLACE, _window, atom_list[NET_WM_DESKTOP],
                     XCB_ATOM_CARDINAL, 32, 1, (const uint32_t[]){0u - 1u});
  _x.change_property(XCB_PROP_MODE_REPLACE, _window,
                     atom_list[NET_WM_STRUT_PARTIAL], XCB_ATOM_CARDINAL, 32, 12,
                     strut);
  ...
  const uint32_t xy[]{static_cast<uint32_t>(_origin_x),
                      static_cast<uint32_t>(_origin_y)};
  ...
  if (WM_CLASS.size()) {
    ...
    _x.change_property(XCB_PROP_MODE_REPLACE, _window, XCB_ATOM_WM_CLASS,
                       XCB_ATOM_STRING, 8, size, wm_class);
  }
...
BarWindow::update_middle(const ModulePixmap& pixmap) {
  ...
  if (largest_offset < _width / 2) {
    size_t middle_offset = std::min(_width / 2 - largest_offset,
                                    static_cast<size_t>(pixmap.size()) / 2);
    _x.copy_area(pixmap.pixmap(), _window, 0, _width / 2 - middle_offset,
