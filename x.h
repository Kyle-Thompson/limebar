...
class X {
 public:
  ...
  // TODO: label
  template <typename T>
  T* get_property(Window win, Atom xa_prop_type, const char* prop_name,
                  unsigned long* size);
  ...
  void draw_ucs2_string(XftDraw* draw, const std::vector<uint16_t>& str,
                        size_t x);
  void draw_ucs2_string_accent(XftDraw* draw, const std::vector<uint16_t>& str,
                               size_t x);
 ...
 public:  // temp
  ...
  struct Fonts {
    ...
    void init(X* x) {
      ...
      for (auto& font : _fonts) font.height = maxh;
    }
    ...
    font_t& drawable_font(const uint16_t c) {
      ...
      // nullptr. If the font can draw the character, return it.
