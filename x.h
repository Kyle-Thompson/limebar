...
class X {
 public:
  ...
  void create_pixmap(xcb_pixmap_t pid, xcb_drawable_t drawable, uint16_t width,
                     uint16_t height);
  void free_pixmap(xcb_pixmap_t pixmap);
  void create_window(xcb_window_t wid, const rgba_t& rgb, int16_t x, int16_t y,
  ...
  struct font_t {
    ...
    auto get_glyph(uint16_t ch) {
      ...
      if (itr == char_to_glyph.end() &&
        ...
        return char_to_glyph.emplace(std::make_pair(ch, create_glyph(ch)))
            .first;
      }
      return itr;
    }

    bool has_glyph(uint16_t ch) { return get_glyph(ch) != char_to_glyph.end(); }

    // TODO: optimize for monospace fonts
    uint16_t char_width(uint16_t ch) { return get_glyph(ch)->second.info.xOff; }
   ...
   private:
    glyph_t create_glyph(uint16_t ch) {
      ...
      FT_UInt glyph_id =
          XftCharIndex(display, xft_ft, static_cast<FcChar32>(ch));
